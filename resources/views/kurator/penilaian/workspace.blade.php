                            ...
                            <div class="d-flex align-items-center mb-4 pb-4 border-bottom">
                                <div class="product-img-wrapper product-img-wrapper--lg mr-3 border d-flex align-items-center justify-content-center">
                                    @if($produkAktif->alternatif->foto_produk)
                                        <img src="{{ Storage::disk('supabase')->url($produkAktif->alternatif->foto_produk) }}" alt="Foto" style="width: 100%; height: 100%; object-fit: cover;">
                                    @else
...
<script>
    $(document).ready(function() {
        ...
        const antreanRaw = @json($antreanProduk->pluck('id_alternatif'));
        const antrean = Array.isArray(antreanRaw) ? antreanRaw.map(Number) : Object.values(antreanRaw).map(Number);
        let isSaving = false;
            ...
            saveCurrentStep().then(() => {
                try {
                    ...
                    if (currentIndex !== -1 && currentIndex < antrean.length - 1) {
                        ...
                        targetUrl = decodeURIComponent(workspaceUrlTemplate).replace(':id', nextId);
                    } else {
                        // All products assessed - go to workspace without ID to trigger semuaDinilai
                        targetUrl = workspaceBaseUrl;
                    }

                    console.log('[Workspace] Navigating to:', targetUrl);
                    window.location.href = targetUrl;
                } catch (e) {
                    console.error('[Workspace] JS Error during navigation:', e);
        ...
        function saveCurrentStep() {
            ...
            return new Promise((resolve, reject) => {
                ...
                if (idKriteria === 'komentar') {
                    ...
                    $.ajax({
                        ...
                        error: function(xhr, textStatus) {
                            console.error('[Workspace] Komentar save error:', textStatus, xhr.status, xhr.responseText);
                            isSaving = false;
                            reject(xhr);
                        }
                    });
                ...
                $.ajax({
                    ...
                    error: function(xhr, textStatus) {
                        console.error('[Workspace] Penilaian save error:', textStatus, xhr.status, xhr.responseText);
                        isSaving = false;
                        reject(xhr);
                    }
                });
